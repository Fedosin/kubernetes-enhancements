...
Independently of those layers, the controller's request path is already rate
limited, which bounds the blast radius on the API server itself. Scale
sub-resource reads and writes and metrics queries -- the latter proxied through
the aggregation layer, and so also API server requests, though not storage ones
(see [Scalability](#scalability)) -- are issued by clients built from the
`kube-controller-manager` client config, each bounded client-side by
`--kube-api-qps` and `--kube-api-burst` (defaults 20 and 30, one token bucket
per client), and server-side by [API Priority and Fairness][].
Aggregate demand above what those limits admit therefore surfaces as longer
scheduling delays inside the HPA controller, discussed next, rather than as API
server saturation.
...
- Real-world usage data -- from the delay-ratio metric on gated clusters,
  ...
  and SIG meetings -- reviewed, in consultation with SIG Scalability, to
  decide whether the lower validation bound should be relaxed below 3s, and
  whether cross-HPA interference warrants workqueue fairness or a
  controller-level budget on aggregate reconcile rate.
...
- The feature has been Beta for at least 2 releases with no major issues
  ...
  at least one release of production usage behind them before the API is
  locked.
...
- **Aggregation-layer calls**: `metrics.k8s.io`, `custom.metrics.k8s.io`, and
  ...
  These consume apiserver request capacity but no storage capacity; the cost of
  serving them lands on metrics-server or the metrics adapter. For the
  request-driven workloads this feature targets, this is where most of the
  additional traffic goes.
